let currentFilters = { mes: null, anio: null };
...
function setupEventListeners() {
  ...
  document.getElementById('btn-todos').addEventListener('click', () => {
    currentFilters = { mes: null, anio: null };
    loadInstalaciones();
  });
...
async function loadSelects() {
  const [encargados, tipos] = await Promise.all([
...
async function loadInstalaciones() {
  ...
  if (currentFilters.mes && currentFilters.anio) {
    url += `?mes=${currentFilters.mes}&anio=${currentFilters.anio}`;
  }
...
function renderTable(data) {
  ...
  if (data.length === 0) {
    tbody.innerHTML = '<tr><td colspan="9" class="empty-state">No hay instalaciones registradas</td></tr>';
    return;
  }
